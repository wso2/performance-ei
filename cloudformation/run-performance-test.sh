...
ei_performance_distribution=""
ei_product_distribution=""
key_file=""
jmeter_distribution=""
oracle_jdk_distribution=""
default_stack_name="ei-performance-test-stack"
stack_name="$default_stack_name"
default_test_name="ei-performance-test"
...
default_ei_ec2_instance_type="t3.small"
ei_ec2_instance_type="$default_ei_ec2_instance_type"
...
function usage() {
    echo ""
    echo "Usage: "
    echo "$0 -f <ei_performance_distribution> -k <key_file> -[-d <results_dir>] [-n <key_name>]"
    echo "   [-b <s3_bucket_name>] [-r <s3_bucket_region>]"
    echo "   [-J <jmeter_client_ec2_instance_type>] [-S <jmeter_server_ec2_instance_type>]"
    echo "   [-B <ei_ec2_instance_type>] [-N <netty_ec2_instance_type>]"
    echo "   [-w <minimum_stack_creation_wait_time>]"
    echo "   [-h] -- [run_performance_tests_options]"
    echo ""
    echo "-f: EI Performance Distribution containing the scripts to run performance tests."
    echo "-k: Amazon EC2 Key File."
    echo "-a: EI Product Distribution (.zip)"
    ...
    echo "-A: Amazon EC2 Instance Type for EI. Default: $default_ei_ec2_instance_type."
...
while getopts "f:k:n:a:j:o:c:m:p:d:b:r:J:S:A:N:z:w:h" opts; do
    case $opts in
    f)
        ei_performance_distribution=${OPTARG}
        ;;
    k)
        key_file=${OPTARG}
        ;;
    n)
        ...
        ;;
    a)
        ei_product_distribution=${OPTARG}
        ;;
    j)
...
if [[ ! -f $ei_performance_distribution ]]; then
    echo "Please provide EI Performance Distribution."
    exit 1
fi

ei_performance_distribution_filename=$(basename $ei_performance_distribution)

if [[ ${ei_performance_distribution_filename: -7} != ".tar.gz" ]]; then
    echo "EI Performance Distribution must have .tar.gz extension"
    exit 1
fi
...
if [[ ! -f $ei_product_distribution ]]; then
    echo "Please provide EI Product zip"
    exit 1
fi

ei_product_distribution_filename=$(basename $ei_product_distribution)

if [[ ${ei_product_distribution_filename: -4} != ".zip" ]]; then
    echo "EI Product must have a .zip extension"
    exit 1
fi
...
if [[ -z $ei_ec2_instance_type ]]; then
    echo "Please provide the Amazon EC2 Instance Type for EI."
    exit 1
fi
...
echo "Extracting EI Performance Distribution to $results_dir"
tar -xf $ei_performance_distribution -C $results_dir
...
ei_performance_distribution=$(realpath $ei_performance_distribution)
ei_product_distribution=$(realpath $ei_product_distribution)
...
ln -s $ei_performance_distribution $temp_dir/$ei_performance_distribution_filename
ln -s $ei_product_distribution $temp_dir/$ei_product_distribution_filename
...
test_parameters_json+=' | .["ei_ec2_instance_type"]=$ei_ec2_instance_type'
...
jq -n \
    ...
    --arg ei_ec2_instance_type "$ei_ec2_instance_type" \
...
create_stack_command="aws cloudformation create-stack --stack-name $stack_name \
    --template-body file://ei_perf_test_cfn.yaml --parameters \
...
./jmeter/create-summary-csv.sh -d results -n ei -p ei -j 2 -g gcviewer.jar

echo "Creating summary results markdown file..."
# Use following to get all column names:
echo "Available column names: "
while IFS= read -r line; do echo -ne " \"$line\""; done < <(./jmeter/create-summary-csv.sh -n "ei" -j 2 -i -x)
...
./jmeter/create-summary-markdown.py --json-files cf-test-metadata.json results/test-metadata.json --column-names \
    ...
    "EI GC Throughput (%)" "Average of EI Memory Footprint After Full GC (M)"
